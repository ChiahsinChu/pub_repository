      ...
      INTEGER::g,h,g1,h1,h2,h3,h4,h5,h6,h7,h8
                  ...
                  DO i=1,9
                      DO j=1,nwat
                          ...
                          IF (d > 1 .AND. d < 3.5) Then
                              dipole1=vector(coor(:,(i-1)*(3*nwat+2*noh)+iwat(j)+1),&
                                      coor(:,(i-1)*(3*nwat+2*noh)+iwat(j)),box)
                              dipole2=vector(coor(:,(i-1)*(3*nwat+2*noh)+iwat(j)+2),&
                                      coor(:,(i-1)*(3*nwat+2*noh)+iwat(j)),box)
                              dipole3=vector(wato(:,k),coor(:,(i-1)*(3*nwat+2*noh)+iwat(j)),box)
                      ...
                      DO j=1,noh
                          ...
                          IF (d > 1 .AND. d < 3.5) Then
                              dipole1=vector(coor(:,(i-1)*(3*nwat+2*noh)+ioh(j)+1),coor(:,(i-1)*(3*nwat+2*noh)+ioh(j)),box)
                              dipole2=vector(wato(:,k),coor(:,(i-1)*(3*nwat+2*noh)+ioh(j)),box)
              ...
              DO i=1,9
                  DO j=1,nwat
                      ...
                      IF (d > 1 .AND. d < 3.5) Then
                          ...
                          dipole2=vector(coor(:,(i-1)*(3*nwat+2*noh)+iwat(j)+2),&
                                  coor(:,(i-1)*(3*nwat+2*noh)+iwat(j)),box)
                          dipole3=vector(oho(:,k),coor(:,(i-1)*(3*nwat+2*noh)+iwat(j)),box)
                          costheta1=(dipole1(1)*dipole3(1)+dipole1(2)*dipole3(2)+dipole1(3)*dipole3(3))&
                                    /sqrt(dipole1(1)**2+dipole1(2)**2+dipole1(3)**2)&
                                    /sqrt(dipole3(1)**2+dipole3(2)**2+dipole3(3)**2)
                          costheta2=(dipole3(1)*dipole2(1)+dipole3(2)*dipole2(2)+dipole3(3)*dipole2(3))&
                  ...
                  DO j=1,noh
                      ...
                      IF (d > 1 .AND. d < 3.5) Then
                          dipole1=vector(coor(:,(i-1)*(3*nwat+2*noh)+ioh(j)+1),&
                                  coor(:,(i-1)*(3*nwat+2*noh)+ioh(j)),box)
                          dipole2=vector(oho(:,k),coor(:,(i-1)*(3*nwat+2*noh)+ioh(j)),box)
                          costheta1=(dipole1(1)*dipole2(1)+dipole1(2)*dipole2(2)+dipole1(3)*dipole2(3))&
                                      /sqrt(dipole1(1)**2+dipole1(2)**2+dipole1(3)**2)&
                                      /sqrt(dipole2(1)**2+dipole2(2)**2+dipole2(3)**2)
                          IF (costheta1 > 0.82) THEN
...
!          WRITE(*,*) g,h1,h2,h3,h4,h5,h6,h7,h8
!          WRITE(*,*) m
