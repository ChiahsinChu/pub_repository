...
def check_fp_task(fp_dir, kw="task.*"):
    """
    If the fp task (cp2k) in dpgen was restarted
    The output file cannot be read (due to the setting in dpdata)
    This function is used for searching the breakpoint 
    """
    l = glob.glob(fp_dir + kw)
    l.sort()
