...
from jxzhu_package.postprocess import 

def gen_dpgen_fp(input_dir, input_file, type_map, output_dir="./"):
    if os.path.exists(output_dir + "iter.000000"):
        print('Directory already exist!')
    else:
        type_list = read_type(input_dir + "type.raw")
        ...
        for i in range(len(coord)):
            os.mkdir(output_dir + "iter.000000/02.fp/task.000." +
                     str(i).rjust(6, '0'))
            atoms = gen_atoms(type_map, type_list, coord[i])
            write(
