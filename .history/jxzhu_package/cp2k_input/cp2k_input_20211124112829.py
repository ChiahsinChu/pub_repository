...
def set_kind(input_dict, elem, basis_set, pp):
    if (elem in input_dict["FORCE_EVAL"]["SUBSYS"]["KIND"]["_"]) == False:
        ...
        input_dict["FORCE_EVAL"]["SUBSYS"]["KIND"]["_"].append(basis_set)
        input_dict["FORCE_EVAL"]["SUBSYS"]["KIND"]["POTENTIAL"].append(pp)
    return input_dict


def set_nosiy_gamma(input_dict, init_idx, fin_idx, nosiy_gamma):
    input_dict["MOTION"]["MD"]["THERMAL_REGION"]["DEFINE_REGION"]["TEMPERATURE"]
    "NOISY_GAMMA_REGION": [],
                    "LIST": []
...
def dpmd(graph, restart=False, stride=1):
