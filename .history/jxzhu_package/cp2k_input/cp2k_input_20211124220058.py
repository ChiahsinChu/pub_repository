...
def set_motion_print_stride(input_dict, stride):
    update_d = {
        "MOTION":{
            "PRINT": {
                "TRAJECTORY": {
...
def energy(pp_dir=" ", charge=0, multiplicity=1, uks=False, kp=False,
           ...
           hartree=False):
    input_dict = copy.deepcopy(default_energy)
    if pp_dir != " ":
...
def geo_opt(pp_dir=" ", charge=0, multiplicity=1, uks=False, kp=False, 
            kp_mp="2 2 1", dip_cor=False, eden=False, mo=False, pdos=False, 
            hartree=False, restart=False, stride=1):
...
def aimd(pp_dir=" ", charge=0, multiplicity=1, uks=False, dip_cor=False, eden=False, 
