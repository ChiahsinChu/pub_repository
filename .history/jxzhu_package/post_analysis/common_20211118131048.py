...
def get_elem_num(atoms, elem_type):
    """
    get the number of given symbol in the atoms

    Args:
        atoms: ASE Atoms object for analysis
        elem_type: symbol of atom
    Return:
        n: the number of given symbol in the atoms
    """
...
def cell_cross_area(atoms):
    ...
    a = cell[0]
    b = cell[1]
    area = a * b * vec_cos(a,b)
    return area


def get_idx_coord(atoms, idx, axis="xyz"):
    coords = atoms.get_positions()
    if axis == "x":
        _coords = coords[:,0]
        idx_coords = []
        for i in idx:
            idx_coords.append(_coords[i])
    elif axis == "y":
        _coords = coords[:,1]
        idx_coords = []
        for i in idx:
        ...
        _coords = np.delete(coords, 2, axis=1)
        idx_coords = []
        for i in idx:
            idx_coords.append(_coords[i])
    elif axis == "xz":
        _coords = np.delete(coords, 1, axis=1)
        idx_coords = []
        for i in idx:
            idx_coords.append(_coords[i]) 
    elif axis == "yz":
        _coords = np.delete(coords, 0, axis=1)
        idx_coords = []
        for i in idx:
            idx_coords.append(_coords[i])
    elif axis == "xyz":
