...
def _gen_afs_slope(atom_soap_data, force_data):
    ...
    for i, i_item in enumerate(atom_soap_data):
        ...
        j = np.argmin(diss)
        afs_slope[i] = np.linalg.norm(force_data[idx_a] - force_data[idx_b]) / np.min(inner_diss) 
    ...
    for idx_a, inner_diss in enumerate(inner_diss_matrix):
        inner_diss[idx_a] = np.max(inner_diss) + 1
        idx_b = np.argmin(inner_diss)
        afs_slope[idx_a] = np.linalg.norm(force_data[idx_a] - force_data[idx_b]) / np.min(inner_diss) 
    return afs_slope
