...
def get_1d_distribution(input_data, xmin, xmax, nbins):
    ...
    Args:
        input_data: array for (x,y) pair [[x1,y1],[x2,y2],...]
    ...
    for data in input_data:
        if data[0] >= xmin + count_bin * bin and data[0] < xmin + (count_bin +
                                                                   1) * bin:
            tmp_data.append(data[1])
        elif data[0] >= xmin + (count_bin + 1) * bin and data[0] < xmin + (
                count_bin + 2) * bin and count_bin < nbins:
