...
def get_atomic_number(type_map):
    ...
    data = new_atoms.get_atomic_numbers()
    data = np.sort(data)
    atomic_number = ' '.join(str(i) for i in data)
...
def gen_desc(type_map, cutoff=6.0, nmax=6, lmax=6, sigma=1.0, soap_type='dscribe'):
    if soap_type == "dscribe":
        ...
        desc_info = "soap cutoff=" + str(cutoff) + " l_max=" + str(lmax) + " n_max=" + str(nmax) + " atom_sigma=" + str(sigma) + " n_Z=" + str(len(type_map)) + " Z={" + atomic_number + "} n_species=" + str(len(type_map)) + " species_Z={" + atomic_number + "}"
        #print(desc_info)
...
def gen_elem_data(atom_data, data_type, type_map, type_list, elem_type):
    ...
    if data_type == "force":
        data = np.reshape(data, (-1, len(type_list), 3))
    if data_type == "soap":
        data = np.reshape(data, (-1, len(type_list), np.shape(data)[-1]))
    elem_data = []
...
def gen_pca_data(data, n_components=2, if_output=True):
    """
    Args:
        data: 2-dimensional soap numpy array (num_soap, dim_soap)
        n_components: arg in sklearn.decomposition.PCA
    """
...
def gen_multi_data(file_list):
    """
    Args: 
        file_list: dir list of `*.npy`
    """
    num_list = []
...
def gen_afs_slope(atom_soap_data, force_data):
    ...
    atom_soap_data = np.reshape(atom_soap_data, (-1, np.shape(atom_soap_data)[-1]))
    force_data = np.reshape(force_data, (-1, 3))
...
def _gen_afs_slope(atom_soap_data, force_data):
    ...
    for i, i_item in enumerate(atom_soap_data):
        diss = np.zeros(len(atom_soap_data))
        for j, j_item in enumerate(atom_soap_data):
            diss[j] = np.linalg.norm(i_item - j_item)
        diss[i] = np.max(diss) + 1
        j = np.argmin(diss)
        afs_slope[i] = np.linalg.norm(force_data[i] - force_data[j]) / np.min(diss) 
    return afs_slope
