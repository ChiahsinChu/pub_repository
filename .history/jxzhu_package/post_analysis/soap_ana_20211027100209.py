...
def _gen_afs_slope(soap_data, force_data):
    ...
    for idx_a, inner_diss in enumerate(inner_diss_matrix):
        inner_diss[idx_a] = np.max(inner_diss) + 1
        idx_b = np.argmin(inner_diss)
        #afs_slope[idx_a] = np.max(np.linalg.norm((force_data[idx_a] - force_data[idx_b]), axis=1)) / np.min(inner_diss) 
        afs_slope[idx_a] = np.linalg.norm(force_data[idx_a] - force_data[idx_b], axis=1) / np.min(inner_diss) 
        #afs_slope[idx_a] = idx_b
    return afs_slope
